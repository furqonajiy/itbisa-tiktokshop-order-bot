...
- **Dispatch spacing (`balance_throttle.py`, duplicated):** `MIN_INTERVAL_HOURS` is the minimum spacing between balance dispatches. **Currently `0` (no throttle): balance fires on every run that touches SKUs**, so a platform that just sold down is rebalanced immediately and stock is never stranded on one platform. Raise above `0` to conserve GitHub Actions minutes at the cost of that freshness. Base SKUs touched while a dispatch is withheld (a positive interval, or a failed dispatch) accumulate in `pending_skus` (`data/balance_throttle.json` on `bot-state`) and flush in one dispatch on the next run — so withholding never drops a SKU (packages are marked processed immediately; `/stock_balance` is idempotent). `_run_throttled_balance` in `main.py` orchestrates: load → `merge_pending` → if `window_open` flush all pending (reset window on success), else defer. Heartbeat shows `⏳ Stock Balance: N SKU menunggu` when deferred. `_format_balance_line` treats `failed` as a count.
