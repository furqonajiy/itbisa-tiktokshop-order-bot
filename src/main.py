...
def _run_throttled_balance(balance):
    ...
    packages are marked processed immediately. Best-effort: never raises.
    """
...
def _do_run(precheck=False):
    ...
    # Dispatch /stock_balance for the base SKUs touched this run, throttled to
    # at most one dispatch per balance_throttle.MIN_INTERVAL_HOURS. SKUs touched
    # while throttled are accumulated and flushed when the window reopens.
    # Best-effort: never fails the order run. Labels are the critical path.
